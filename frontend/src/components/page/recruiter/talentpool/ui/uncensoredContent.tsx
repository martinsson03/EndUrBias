...
import { ChangeApplicationState, GetUncensoredApplications } from "@/lib/client/services/applicationService";
...
type UncensoredContentProps = {
    job: JobViewModel
};

export default function UncensoredContent({ job }: UncensoredContentProps) {
    const [applications, setApplications] = useState<CVViewModel[] | null>(null);

    
    useEffect(() => {
        async function fetchApplications() {
            const data = await GetUncensoredApplications(job.id);
        ...
        <PaginatedContainer columns={5}>
            { applications && applications.length > 0 && applications.map((app, i) => (
                <Card key={i} className="hover:cursor-pointer p-2" onClick={() => openDialog("", "", cvDialog(app), "60%", "100%")}>
                    <p className="text-center">{ app.displayname }</p>
                </Card>
            ))}
        </PaginatedContainer>
    );
